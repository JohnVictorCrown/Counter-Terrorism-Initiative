...
DB_PATH = Path(__file__).resolve().parent.parent / "databases" / "leads.db"
...
VALID_TYPES = ["Military", "Law Enforcement", "Intelligence", "State Police",
...
def add_lead(data: dict) -> str:
    ...
    if not org_type:
        raise ValueError("type is required. Choose from: " + ", ".join(VALID_TYPES))
    lid = str(uuid.uuid4())
    db = get_db()
    db.execute(
        ...
        (lid, data.get("company", ""), data.get("contact_name", ""),
         ...
         data.get("tier", "3"), org_type, data.get("vertical", ""),
...
def update_lead(lid: str, data: dict):
    if "type" in data and not data["type"].strip():
        raise ValueError("type cannot be empty. Choose from: " + ", ".join(VALID_TYPES))
    fields = []
    params = []
...
def import_csv(csv_path: str) -> int:
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if not company:
                continue
            org_type = (row.get("Type") or "").strip()
            if not org_type:
                skipped += 1
                print(f"  Skipped '{company}' — type is required")
                continue
            lid = str(uuid.uuid4())
            db.execute(
                """INSERT INTO leads (id, company, contact_name, email, phone, website,
        ...
        db.close()
    if skipped:
        print(f"  Warning: {skipped} row(s) skipped because type column was empty")
    return count
...
def cmd_add(args=None):
    ...
    data["type"] = input("  Type *: ").strip()
    if not data["type"]:
        print("Type is required.")
        return
    data["vertical"] = input("  Vertical (country/region/topic): ").strip()
